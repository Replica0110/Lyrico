...
void JMetadataBuilder::setId3v2(TagLib::ID3v2::Tag &tag) {
    ...
    for (auto frame : tag.frameList()) {
        if (auto txxxFrame =
            ...
            const TagLib::String& description = *begin;
            ...
            TagLib::String key = frame->frameID();
            // 获取歌词文本
            TagLib::String text = lyricsFrame->text();

            if (!text.isEmpty()) {
                // 将歌词添加到 id3v2 map 中
                id3v2.add_id(key, text);
            }
        }
            // ----------------------------------------
...
jobject JMetadataBuilder::build() {
    ...
    JClassRef jPropertiesClass {
            env,
            "com/lonx/audiotag/internal/Properties"
    };

    jmethodID jPropertiesInitMethod =
            jPropertiesClass.method(
                    "<init>",
                    "(Ljava/lang/String;JIIILcom/lonx/audiotag/internal/Properties$ReadStyle;IILcom/lonx/audiotag/internal/Properties$BitrateMode;Ljava/lang/String;Ljava/lang/String;Ljava/lang/Integer;)V"
            );

    if (jPropertiesInitMethod == nullptr) {
    ...
    // ReadStyle.AVERAGE
    jclass readStyleClass = env->FindClass(
            "com/lonx/audiotag/internal/Properties$ReadStyle");

    jfieldID averageField = env->GetStaticFieldID(
            readStyleClass,
            "AVERAGE",
            "Lcom/lonx/audiotag/internal/Properties$ReadStyle;"
    );

    jobject readStyle =
            env->GetStaticObjectField(readStyleClass, averageField);


    // BitrateMode.CBR
    jclass bitrateModeClass = env->FindClass(
            "com/lonx/audiotag/internal/Properties$BitrateMode");

    jfieldID cbrField = env->GetStaticFieldID(
            bitrateModeClass,
            "CBR",
            "Lcom/lonx/audiotag/internal/Properties$BitrateMode;"
    );

    jobject bitrateMode =
            env->GetStaticObjectField(bitrateModeClass, cbrField);


    // String
    JStringRef jMimeType { env, this->mimeType };
    JStringRef jCodec { env, "" };
    JStringRef jEncoder { env, "" };
    ...
    jobject bitsPerSample =
            env->NewObject(integerClass, integerInit, 16);


    JObjectRef jProperties {
            env,
            env->NewObject(
                    *jPropertiesClass,
                    jPropertiesInitMethod,

                    *jMimeType,                                   // String
                    (jlong) properties->lengthInMilliseconds(),   // long durationMs
                    properties->bitrate(),                        // int bitrate
                    properties->sampleRate(),                     // int sampleRate
                    properties->channels(),                       // int channels

                    readStyle,                                    // ReadStyle
                    properties->lengthInSeconds(),                // int
                    (jint) properties->lengthInMilliseconds(),    // int
    ...
    JClassRef jMetadataClass {
            env,
            "com/lonx/audiotag/internal/Metadata"
    };

    jmethodID jMetadataInitMethod =
            jMetadataClass.method(
                    "<init>",
                    "(Ljava/util/Map;Ljava/util/Map;Ljava/util/Map;[BLcom/lonx/audiotag/internal/Properties;)V"
            );
